...
bool ListaCircular<T>::InsereNaPosicao(int pos, T _valor) {
	...
	auto atual = this->cabeca;
	int posAtual = 0;
	while (atual->getNext() != this->cabeca && posAtual < pos-1) {
		atual = atual->getNext();
		posAtual++;
	}
	if(posAtual == this->size()-1) {
		return InsereNoFinal(_valor);
	}
...
bool ListaCircular<T>::RemoveNaPosicao(int pos) {
	if (pos<0) return false;
	if (pos==0)	return RemoveNoInicio();
	if (pos==this->size()-1) return RemoveNoFinal();
...
std::ostream& operator<< ( std::ostream& o, ListaCircular<T> const &l) {
	int quantidade = l.tamanho;
	if(quantidade > 0 ){
		auto atual = l.cabeca;
		
		quantidade++;
	
		while ( atual->getNext() != l.cabeca) {
			o << atual->getValor() << " ";
			atual = atual->getNext();
		}
		o << atual->getValor() << " ";
